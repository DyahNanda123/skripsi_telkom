<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content" style="border-radius: 20px; padding: 15px;">
        <form action="{{ url('/pengguna/' . $user->id . '/destroy_ajax') }}" method="POST" id="form-delete">
            @csrf
            @method('DELETE')
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.status) {
                        ...
                        // Reload tabel Pengguna
                        if (typeof dataPengguna !== 'undefined') {
                            dataPengguna.ajax.reload(null, false);
                        }
                    } else {
                ...
                error: function(xhr) {
                    btn.html(btnAsli).prop('disabled', false); 
                    Swal.fire({ icon: 'error', title: 'Error Server', text: 'Terjadi kesalahan sistem.' });
                }
            });
        });
    });
